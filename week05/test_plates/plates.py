...
# is_valid still expects a str as input and
# returns True if that str meets all requirements and False if it does not, but main is
# only called if the value of __name__ is "__main__":
...
def main():
    plate = input("Plate: ")
    x = is_valid(plate)
    # print("Printing for testing purposes" + str(x))
...
def is_valid(s):
    ...
    for char in s[2:]:  # We start checking from the third character
        if char.isnumeric():
            if char == '0' and not digit_presence:
